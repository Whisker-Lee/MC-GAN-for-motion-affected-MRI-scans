...
class Deblur_Net():
    ...
    def generator(self, x, reuse = False, name = 'generator'):
        ...
        with tf.variable_scope(name_or_scope = name, reuse = reuse):
            ...
            for i in range(self.num_of_down_scale):
                x = self.down_scaling_feature(name = 'down_%02d'%i, x = x, n_feats = self.n_feats * (2 ** (i)))
            ...
            mx = tf.reduce_max(x, axis=[1,2,3])
            mi = tf.reduce_min(x, axis=[1,2,3])

            mx0=tf.reshape(mx,[-1,1,1,1])
            mi0=tf.reshape(mi,[-1,1,1,1])
    ...
    def build_graph(self):
        ...
        self.GP_loss = tf.reduce_mean(tf.square(tf.sqrt(tf.reduce_mean(tf.square(gradient), axis = [1, 2, 3])) - 1))
        
        self.d_loss_real = - tf.reduce_mean(self.real_prob)
        self.d_loss_fake = tf.reduce_mean(self.fake_prob)
        
        if self.mode == 'train':
            self.vgg_net = Vgg19(self.vgg_path)
            self.vgg_net.build(tf.concat([label, self.gene_img], axis = 0))
            self.content_loss = tf.reduce_mean(tf.square(self.vgg_net.conv3_3[self.batch_size:] - self.vgg_net.conv3_3[:self.batch_size]))


            self.mse_loss = tf.reduce_mean((self.gene_img - label) ** 2)

            self.D_loss = self.d_loss_real + self.d_loss_fake + 10.0 * self.GP_loss
            self.G_loss = - 100 * self.d_loss_fake + 0.0001 * self.content_loss + 50000 * self.mse_loss 
        ...
        mx = tf.reduce_max(self.blur, axis=[1,2,3])
        mx = tf.reshape(mx,[-1,1,1,1])
        self.blur2 = self.blur / mx

        mx2 = tf.reduce_max(self.output2, axis=[1,2,3])
        mx2 = tf.reshape(mx2,[-1,1,1,1])
        self.output2 = self.output2 / mx2 

        self.true_out = tf.placeholder(name = "true_out", shape = [None, None, None, self.channel], dtype = tf.float32)
        mx3 = tf.reduce_max(self.true_out, axis=[1,2,3])
        mx3 = tf.reshape(mx3,[-1,1,1,1])
        self.true_out2 = self.true_out / mx3

        rib = 256
        corrected_mse = tf.reduce_sum((self.output2 - self.true_out2) ** 2, [1,2,3])/(rib*rib*1)
        blur_mse = tf.reduce_sum((self.blur2 - self.true_out2) ** 2, [1,2,3])/(rib*rib*1)
        self.corrected_rmse=corrected_mse**0.5
        self.blur_rmse=blur_mse**0.5

        self.blur_psnr = 10*tf.log((1/blur_mse))/tf.log(tf.constant(10., dtype=tf.float32))
        self.corrected_psnr = 10*tf.log((1/corrected_mse))/tf.log(tf.constant(10., dtype=tf.float32))
