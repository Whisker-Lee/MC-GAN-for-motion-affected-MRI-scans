...
def test_only(args, model, sess, saver):
    ...
    if args.in_memory :
        ...
        a = []
        for i in range(len(lap_var_corrected_all)):
            a.append([sc_blur_all[i], sc_corre_all[i]])
        a = pd.DataFrame(a, columns = ['b','c'])
        a.to_csv('./Data/abide_100_y_all.csv',sep=',',index=False,header=True)
